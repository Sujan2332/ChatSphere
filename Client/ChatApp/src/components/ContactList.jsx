...
const ContactList = ({ setUser }) => {
  ...
  const [token, setToken] = useState(null);  // Add token state to manage the token
  ...
  // Fetch the token from localStorage on initial load
  useEffect(() => {
  ...
  }, [token]);  // Trigger fetching when token changes
